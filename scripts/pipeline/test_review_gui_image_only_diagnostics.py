...
class ReviewGuiImageOnlyDiagnosticsTests(unittest.TestCase):
    ...
    def test_load_manual_vlm_models_for_gui_uses_only_vlm_entries_from_shared_models_list(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repo_root = Path(temp_dir)
            config_path = repo_root / "conf" / "vlm_models.yaml"
            config_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        self.assertEqual([model["VLM_NAME"] for model in models], ["Preset A"])
