...
DEFAULT_MODEL_NAME = "microsoft/Florence-2-base-ft"
DEFAULT_MAX_NEW_TOKENS = 96


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Caption stage photos with Florence-2 and write one /tmp text file per image.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    return run_caption_cli(
        ...
        description="Caption Florence scenes",
        build_captioner=lambda: build_florence_captioner(args.model_name, args.device, args.max_new_tokens),
    )
