...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build chunked transcript demo files for semantic announcement extraction."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help="Start a new chunk when the silence gap exceeds this value. Default: 60",
    )
    parser.add_argument(
        "--summary-output",
        default="semantic_announcement_demo.csv",
        help="Summary CSV filename inside workspace or absolute path. Default: semantic_announcement_demo.csv",
    )
    parser.add_argument(
        "--jsonl-output",
        default="semantic_announcement_demo.jsonl",
        help="Full JSONL filename inside workspace or absolute path. Default: semantic_announcement_demo.jsonl",
    )
...
def extract_candidates_from_segment(text: str) -> List[int]:
    ...
    for index, token in enumerate(tokens):
        ...
        if parsed is None:
            continue
        performance_number, _, _ = parsed
        if performance_number > 0:
            matches.append(performance_number)
    return matches
...
def build_prompt(chunk: Dict) -> str:
    ...
    for segment in segments:
        lines.append(
            f"- {segment['segment_start_local']} -> {segment['segment_end_local']}: {segment['text']}"
        )
    transcript = "\n".join(lines)
    return (
        "You extract structured dance-competition announcements from Polish transcripts.\n"
        ...
        "}\n"
        "Rules:\n"
        "- Prefer explicit start numbers such as 'numer startowy 279' over ordinals such as 'szosty w tej kategorii'.\n"
        "- Ceremony result reading is not a performance announcement.\n"
        "- Ignore unrelated speech.\n"
        "- Use only evidence present inside this chunk.\n\n"
        f"Chunk ID: {chunk['chunk_id']}\n"
        f"Stream: {chunk['stream_id']}\n"
        f"Chunk start: {chunk['chunk_start_local']}\n"
        f"Chunk end: {chunk['chunk_end_local']}\n"
        "Transcript lines:\n"
        f"{transcript}\n"
    )
...
def build_summary_table(summary_rows: Sequence[List[str]]) -> Table:
    table = Table(title="Semantic Announcement Demo Summary", expand=False)
    ...
    table.add_column("Regex Hits", justify="right", style="red")
    table.add_column("First Chunk", style="white")
    table.add_column("Last Chunk", style="white")
...
def main() -> int:
    ...
    if not summary_output.is_absolute():
        summary_output = workspace_dir / summary_output
    jsonl_output = Path(args.jsonl_output)
    if not jsonl_output.is_absolute():
        jsonl_output = workspace_dir / jsonl_output
    ...
    stream_segments: Dict[str, List[Dict]] = {stream_id: [] for stream_id in selected_streams}
                    ...
                    stream_segments[stream_id].append(
                        {
                            ...
                            "text": text,
                            "regex_candidate_numbers": extract_candidates_from_segment(text),
                        }
                    )
    ...
    for stream_id in selected_streams:
        segments = sorted(
            ...
            key=lambda item: (item["segment_start_local"], item["filename"], item["segment_index"]),
        )
        chunks: List[List[Dict]] = []
        current_chunk: List[Dict] = []
        previous_end: Optional[datetime] = None
        for segment in segments:
            segment_start = parse_local_datetime(segment["segment_start_local"])
            segment_end = parse_local_datetime(segment["segment_end_local"])
            if segment_start is None or segment_end is None:
                continue
            if previous_end is not None and (segment_start - previous_end).total_seconds() > args.gap_seconds:
                if current_chunk:
        ...
        first_chunk = ""
        last_chunk = ""
        for chunk_index, chunk_segments in enumerate(chunks, 1):
            chunk_start_local = chunk_segments[0]["segment_start_local"]
            chunk_end_local = chunk_segments[-1]["segment_end_local"]
            chunk_start = parse_local_datetime(chunk_start_local)
            chunk_end = parse_local_datetime(chunk_end_local)
            chunk_duration_seconds = 0.0
            if chunk_start is not None and chunk_end is not None:
                chunk_duration_seconds = (chunk_end - chunk_start).total_seconds()

            candidate_numbers: List[int] = []
            filenames: List[str] = []
            clip_paths: List[str] = []
            for segment in chunk_segments:
                candidate_numbers.extend(segment["regex_candidate_numbers"])
                if segment["filename"] not in filenames:
                    filenames.append(segment["filename"])
                if segment["clip_path"] not in clip_paths:
                    clip_paths.append(segment["clip_path"])

            regex_hits += len(candidate_numbers)
            if not first_chunk or chunk_start_local < first_chunk:
                first_chunk = chunk_start_local
            if not last_chunk or chunk_start_local > last_chunk:
                last_chunk = chunk_start_local

            full_text = " ".join(segment["text"] for segment in chunk_segments).strip()
            text_preview = full_text[:180]
            if len(full_text) > 180:
                text_preview += "..."
            chunk_id = f"{stream_id}:{chunk_index:04d}"
            ...
            chunk_rows.append(
                {
                    ...
                    "chunk_end_local": chunk_end_local,
                    "chunk_duration_seconds": f"{chunk_duration_seconds:.3f}",
                    "segment_count": str(len(chunk_segments)),
                    "clip_count": str(len(filenames)),
                    "regex_candidate_count": str(len(candidate_numbers)),
                    "regex_candidate_numbers": ",".join(str(number) for number in candidate_numbers),
            ...
            chunk_payload = {
                ...
                "clip_count": len(filenames),
                "filenames": filenames,
                "clip_paths": clip_paths,
                "regex_candidate_numbers": candidate_numbers,
                "text": full_text,
                "segments": chunk_segments,
            }
            chunk_payload["prompt"] = build_prompt(chunk_payload)
            jsonl_rows.append(chunk_payload)
    ...
    chunk_rows.sort(key=lambda row: (row["stream_id"], row["chunk_start_local"], row["chunk_id"]))
    jsonl_rows.sort(key=lambda row: (row["stream_id"], row["chunk_start_local"], row["chunk_id"]))

    written_csv = write_csv(summary_output, SUMMARY_HEADERS, chunk_rows)
    written_jsonl = write_jsonl(jsonl_output, jsonl_rows)

    console.print(build_summary_table(summary_rows))
    console.print(f"[green]Wrote {written_csv} demo chunk rows to {summary_output}[/green]")
    console.print(f"[green]Wrote {written_jsonl} demo chunk payloads to {jsonl_output}[/green]")
    return 0
